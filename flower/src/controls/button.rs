...
use glow::{HasContext, LINEAR, NEAREST, REPEAT, RGB, RGBA, TEXTURE_2D, TEXTURE_MAG_FILTER, TEXTURE_MIN_FILTER, TEXTURE_WRAP_S, TEXTURE_WRAP_T, UNSIGNED_BYTE};
...
impl Control for Button {
    fn on_draw(&mut self, gl: &mut Draw) {
        ...
        unsafe {
            ...
            // 加载并生成纹理
            let image = image::open("flower/resource/test.jpg").expect("Failed to load texture");
            let data = image.as_bytes();


            gl.tex_image_2d(TEXTURE_2D, 0, RGB as i32, image.width() as i32, image.height() as i32, 0, RGB, UNSIGNED_BYTE, Some(data));
